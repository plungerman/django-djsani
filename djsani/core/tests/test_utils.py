...
class CoreUtilsTestCase(TestCase):
    ...
    def test_get_manager(self):
        """Obtain the current data manager."""
        logger.debug("fetch the current user manager")
        logger.debug(seperator())
        cid = self.user.id
        manager = StudentMedicalManager.objects.filter(college_id=cid).filter(
        ...
        past_man = StudentMedicalManager.objects.filtery(
            college_id=cid,
        ).order_by('-id').first()
        ...
        if past_man:
            ...
            if past_man.cc_athlete_sicklecell_waiver:
                ...
                sc = Sicklecell.objects.filter_by(
                    college_id=cid,
                ).order_by('-id').first()
                if sc.proof:
                    sicklecell = True
        else:
            logger.debug("no past manager")
        # create new manager
